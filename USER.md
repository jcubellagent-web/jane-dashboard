# USER.md - About Your Human

- **Name:** Josh
- **What to call them:** Josh
- **Pronouns:** *(not specified yet)*
- **Timezone:** Eastern (America/New_York)
- **Location:** New York
- **X/Twitter:** @jcubnft (verified, 1,054 followers, joined March 2021)
- **TikTok:** @degencollector

## Interests & Passions

### Sports
- **HUGE Patriots fan** 🏈 — follows Drake Maye content, uses #gopats
- Active during NFL season, collects Panini NFT sports cards

### Crypto & Web3
- Early adopter mentality
- "Chain agnostic" — not tied to any single blockchain
- Interested in stablecoins, DeFi, NFTs
- Follows crypto news (Watcher.Guru, etc.)
- Has Solana experience (sent me 0.5 SOL, knows memecoin trading)

### Digital Collectibles
- Panini NFT collector (sports cards)
- TikTok content creator for pack openings (@degencollector)

## Personality & Style

- Bio: "Early Adopter | Here for fun and to learn | Chain agnostic | NFA, posts are my opinions at that point in time 🫡"
- Casual, fun approach to crypto — "here for fun and to learn"
- Uses NFA (Not Financial Advice) disclaimer
- Prefers to build trust gradually — start professional, mature from there

## Wallets

- **Solana:** `6EYvnXTGFj5HQzLAJMYs4EpYnzQ6A4gUVrG5vncP96h8`

## Context

- First day together (January 31, 2026)
- Set up: WhatsApp, Gmail, TikTok, X account for me, Phantom wallet
- Did paper trading session together — learned about SOL trenches

---

*Updated: January 31, 2026*
