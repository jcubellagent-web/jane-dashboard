#!/usr/bin/env python3
"""
Drake Maye Hunt EP.8 - V5 FINAL
Apply overlays AFTER concatenation to avoid timing issues
"""
...
OUTPUT = f"{WORK_DIR}/EP8_V5.mp4"

WIDTH = 384
HEIGHT = 848
FPS = 30

def draw_text_outline(draw, text, x, y, font, fill, outline=(0,0,0), width=3):
...
def create_overlay_png(lines, filename):
    """Create overlay PNG with multiple lines"""
    ...
    for text, font_size, color in lines:
        ...
        y_offset += 35 if font_size == 'lg' else 28
    
    img.save(filename, 'PNG')

def main():
    print("=" * 50)
    print("Drake Maye Hunt EP.8 - V5 FINAL")
    print("=" * 50)
    
    # Step 1: Create title card (3s)
    print("\n1. Creating title card...")
    title_jpg = f"{WORK_DIR}/v5_title.jpg"
    create_title_card().save(title_jpg, quality=95)
    title_mp4 = f"{WORK_DIR}/v5_title.mp4"
    subprocess.run([
    ...
    main_mp4 = f"{WORK_DIR}/v5_main.mp4"
    subprocess.run([
        'ffmpeg', '-y', '-i', SOURCE,
    ...
    results_jpg = f"{WORK_DIR}/v5_results.jpg"
    create_results_card().save(results_jpg, quality=95)
    results_mp4 = f"{WORK_DIR}/v5_results.mp4"
    subprocess.run([
    ...
    # Step 5: Create overlays and apply to concatenated video
    # Timing: Title=0-3s, Main=3-27s, Results=27-31s
    # In main segment: Tip@9s, Howie@13s, Dre@17s, Marquise@21s
    # In final video: Tip@12s, Howie@16s, Dre@20s, Marquise@24s
    print("5. Applying overlays...")
    ...
    for i, (_, start, end) in enumerate(png_files):
        out = f'v{i}'
        filters.append(f"[{prev}][{i+1}:v]overlay=0:{HEIGHT-110}:enable='between(t,{start},{end})'[{out}]")
        prev = out
    
    overlay_mp4 = f"{WORK_DIR}/v5_overlay.mp4"
    cmd = ['ffmpeg', '-y'] + inputs + [
